...
def boxes_to_corners_3d_v2xsim(boxes3d, order):
    """
        4 -------- 5
       /|         /|
      7 -------- 6 .
    ...
    if order == 'hwl':
        boxes3d_ = boxes3d[:, [0, 1, 2, 5, 4, 3, 6, 7]]
    ...
    corners3d = boxes3d_[:, None, 3:6].repeat(1, 8, 1) * template[None, :, :]
    corners3d = common_utils.rotate_points_along_z_v2xsim(corners3d.view(-1, 8, 3),
                                                   boxes3d_[:, 6],boxes3d_[:,7]).view(-1, 8,
                                                                        3)
    corners3d += boxes3d_[:, None, 0:3]
...
def draw_projected_box3d(image,qs,color=(0,255,0),thickness=2):
    ...
    for q in qs:
        for k in range(0,4):
            i,j = k,(k+1)%4
            cv2.line(image, (q[i, 0], q[i, 1]), (q[j, 0], q[j, 1]), color, thickness)
            i, j = k + 4, (k + 1) % 4 + 4
            cv2.line(image, (q[i, 0], q[i, 1]), (q[j, 0], q[j, 1]), color, thickness)
