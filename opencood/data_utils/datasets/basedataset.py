...
class BaseDataset(Dataset):
    ...
    def reform_camera_param(self, cav_content, ego_content, timestamp):
        ...
        for i in range(4):
            camera_coords = cav_params['camera%d' % i]['cords'] #世界坐标
            camera_extrinsic = np.array(
                cav_params['camera%d' % i]['extrinsic'])#lidar到camera的外参
            camera_extrinsic_to_ego_lidar = x1_to_x2(camera_coords,
                                                     ego_lidar_pose)#相机世界坐标到自车雷达的外参矩阵
            camera_extrinsic_to_ego = x1_to_x2(camera_coords,
                                               ego_pose)#相机世界坐标到自车的外参矩阵
            ego_to_camera_extrinsic = x1_to_x2(ego_pose,camera_coords)
