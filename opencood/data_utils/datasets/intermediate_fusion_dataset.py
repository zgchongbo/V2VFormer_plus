...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            if distance > opencood.data_utils.datasets.COM_RANGE:
                continue
            origin_img.append(selected_cav_processed['camera']['origin_data'])
            ...
            camera2lidar.append(
                selected_cav_processed['camera']['cam2lidar'])
            lidar2img.append(
                selected_cav_processed['camera']['lidar2img'])
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        for camera_id, camera_data in selected_cav_base['camera_np'].items():
            ...
            camera_data = self.pre_processor.preprocess_camera(camera_data) 
            camera_intrinsic = \
        ...
        camera_dict = {
            ...
            'cam2lidar':np.stack(all_camera_tolidar),
            'lidar2img':np.stack(all_lidar_toimg),
        }
    ...
    def collate_batch_train(self, batch):
        ...
        origin_img_all_batch = []
        cam_rgb_all_batch = []
        cam_to_ego_all_batch = []
        cam_intrinsic_all_batch = []

        cam_to_lidar_all_batch = []
        lidar_to_img_all_batch = []
        ...
        for i in range(len(batch)):
            ...
            cam_rgb_all_batch.append(ego_dict['camera_data'])
            origin_img_all_batch.append(ego_dict['origin_img'])
            cam_intrinsic_all_batch.append(ego_dict['camera_intrinsic'])
            cam_to_ego_all_batch.append(ego_dict['camera_extrinsic'])
            cam_to_lidar_all_batch.append(ego_dict['camera2lidar'])
            lidar_to_img_all_batch.append(ego_dict['lidar2img'])
        ...
        origin_img_all_batch = torch.from_numpy(np.concatenate(origin_img_all_batch,axis=0)).unsqeezze(1).float()
        cam_rgb_all_batch = torch.from_numpy(
            np.concatenate(cam_rgb_all_batch, axis=0)).unsqueeze(1).float()  # from_numpy将np转为torch
        ...
        cam_to_lidar_all_batch = torch.from_numpy(
            np.concatenate(cam_to_lidar_all_batch, axis=0)).unsqueeze(1).float()
        lidar_to_img_all_batch = torch.from_numpy(
            np.concatenate(lidar_to_img_all_batch, axis=0)).unsqueeze(1).float()
        transformation_matrix_all_batch = \
