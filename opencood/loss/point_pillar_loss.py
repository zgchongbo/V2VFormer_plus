...
class PointPillarLoss(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        cls_weights = (negative_cls_weights + 1.0 * positives).float() #正样本为2，负样本为1
        reg_weights = positives.float()

        pos_normalizer = positives.sum(1, keepdim=True).float()
        reg_weights /= torch.clamp(pos_normalizer, min=1.0) #除以正样本数
    ...
    def cls_loss_func(self, input: torch.Tensor,
        ...
        alpha_weight = target * self.alpha + (1 - target) * (1 - self.alpha)#0-0.75 1-0.25
