...
class BEVEmbedding(nn.Module):
    def __init__(
        ...
        h = bev_height // (2 ** len(decoder_blocks)) #2的3次方
        ...
        # map from bev coordinates to ego frame  将BEV坐标映射到自车坐标
        # 3 3
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, k, v, skip=None):
        ...
        q = rearrange(q, 'b n d H W -> b n (H W) d') #(5,4,1024,128) 32*32
        k = rearrange(k, 'b n d h w -> b n (h w) d')#(5,4,4096,128) 64*64
...
class CrossViewAttention(nn.Module):
    ...
    def forward(
        ...
        w_embed = self.bev_embed(world[None])  # 1 d H W  这里生成的就是自车下的bev
        ...
        # Expand + refine the BEV embedding (5,4,128,32,32)是全局的query
