...
def generate_grid(height: int, width: int):
    ...
    indices = F.pad(indices, (0, 0, 0, 0, 0, 1), value=1)                   # 3 h w  对通道扩充一维
...
class Attention(nn.Module):
    def __init__(
        ...
        # relative positional bias 相对位置误差
...
class CrossWinAttention(nn.Module):
    def __init__(self, dim, heads, dim_head, qkv_bias, rel_pos_emb=False, norm=nn.LayerNorm):
        ...
        self.scale = dim_head ** -0.5# 不知道是用来干嘛的

        self.heads = heads #4
        self.dim_head = dim_head #32
        self.rel_pos_emb = rel_pos_emb

        self.to_q = nn.Sequential(norm(dim), nn.Linear(dim, heads * dim_head, bias=qkv_bias)) #128
        self.to_k = nn.Sequential(norm(dim), nn.Linear(dim, heads * dim_head, bias=qkv_bias))
        self.to_v = nn.Sequential(norm(dim), nn.Linear(dim, heads * dim_head, bias=qkv_bias))

        self.proj = nn.Linear(heads * dim_head, dim)#对应于Bevformer的out_proj
    ...
    def forward(self, q, k, v, skip=None):
        ...
        # flattening
        q = rearrange(q, 'b n x y w1 w2 d -> b (x y) (n w1 w2) d')#(3,64,1024,128)
        k = rearrange(k, 'b n x y w1 w2 d -> b (x y) (n w1 w2) d')#(3,64,1024,128)
        ...
        q = rearrange(q, 'b ... (m d) -> (b m) ... d', m=self.heads, d=self.dim_head) #(m,d)->(4,32)
        ...
        att = dot.softmax(dim=-1)#softmax(q*k)
        ...
        z = self.proj(a)#对多头进行融合利用linear

        # reduce n: (b n X Y W1 W2 d) -> (b X Y W1 W2 d)
        z = z.mean(1)  # for sequential usage, we cannot reduce it! 对多视图进行融合
...
class CrossViewSwapAttention(nn.Module):
    def __init__(
        ...
        self.register_buffer('image_plane', image_plane, persistent=False)#CrossViewSwapAttention继承自module的方法，定义一组参数，该组参数的特别之处在于：模型训练时不会更新（即调用 optimizer.step() 后该组参数不会变化，只可人为地改变它们的值），但是保存模型时，该组参数又作为模型参数不可或缺的一部分被保存,当然这里persistent=False因此不会保存到static_dict中
        ...
        if self.bev_embed_flag: #只对第一个尺度进行bev位置编码
            self.bev_embed = nn.Conv2d(2, dim, 1)
        self.img_embed = nn.Conv2d(4, dim, 1, bias=False)#对图像进行编码
        self.cam_embed = nn.Conv2d(4, dim, 1, bias=False)

        self.q_win_size = q_win_size[index]
        self.feat_win_size = feat_win_size[index]
        self.rel_pos_emb = rel_pos_emb #False，还没有实现的，不知道功能
        ...
        self.mlp_1 = nn.Sequential(nn.Linear(dim, 2 * dim), nn.GELU(), nn.Linear(2 * dim, dim))#FFN
    ...
    def forward(
        ...
        pixel = self.image_plane       #像素平面 (64,64)                                         # b n 3 h w
        _, _, _, h, w = pixel.shape

        c = E_inv[..., -1:]                                                     # b n 4 1   这是平移变换
        c_flat = rearrange(c, 'b n ... -> (b n) ...')[..., None]                # (b n) 4 1 1
        c_embed = self.cam_embed(c_flat)        #相机编码 需要                                # (b n) d 1 1
        ...
        # pad divisible
        key = self.pad_divisble(key, self.feat_win_size[0], self.feat_win_size[1]) #pad k,v到可以整除窗口
...
class FAXModule(nn.Module):
    def __init__(
        ...
        middle = config['middle']#[2, 2, 2]定义的是layer的层数
        dim = config['dim'] #定义CrossViewSwapAttention的输出维度
            ...
            if i < len(middle) - 1:
                downsample_layers.append(nn.Sequential(
                    nn.Sequential(
                        ...
                        nn.PixelUnshuffle(2), #一种下采样的方式，效果应该比普通插值好，上采样的out=C*2*2，2是scale_factor
    ...
    def forward(self, batch):
        ...
        # lidar2img =rearrange(batch['lidar2img'],
        #                   'b l m h w -> (b l) m h w')
        I_inv = \
        ...
        x = repeat(x, '... -> b ...', b=b * l)  # b*l d H W  复制3份给3辆CAVS
        ...
        for i, (cross_view, feature, layer) in \
            ...
            x = layer(x)
            if i < len(features)-1:#最后一次不用下采样
                down_sample_block = self.downsample_layers[i]
                x = down_sample_block(x)

        x = self.self_attn(x) #简单的多头attention模块
