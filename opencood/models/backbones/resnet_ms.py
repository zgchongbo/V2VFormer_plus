...
class ResnetEncoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.num_layers = params['num_layers'] #34
        self.pretrained = params['pretrained'] #True
        image_height = params['image_height']
        image_width = params['image_width']
        self.idx_pick = params['id_pick']# 要1，2，3层的输出
        ...
        dummy = torch.rand(1, 1, 1, image_height, image_width, 3) #用rand制造了假的picture数据传入网络中，拿到前向输出的形状
        output_shapes = [x.shape for x in self(dummy)] #[torch.Size([1, 1, 1, 128, 64, 64]), torch.Size([1, 1, 1, 256, 32, 32]), torch.Size([1, 1, 1, 512, 16, 16])]
    ...
    def forward(self, input_images):
        ...
        input_images = input_images.view(b * l * m, h, w, c) #输入时batch要合并
        # b, h, w, c -> b, c, h, w
        input_images = input_images.permute(0, 3, 1, 2).contiguous()

        x = self.encoder.conv1(input_images) #(64,256,256)
        x = self.encoder.bn1(x)
        x = self.encoder.relu(x)

        x0 = self.encoder.layer1(self.encoder.maxpool(x))#经过池化后(64,128,128),在layer中尺寸不变
        x1 = self.encoder.layer2(x0)#（128，64，64）
        x2 = self.encoder.layer3(x1)#（256，32，32）
        x3 = self.encoder.layer4(x2)#（512，16，16）
